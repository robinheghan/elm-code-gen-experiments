...
var author$project$Bench$foldl = F3(_foldl);
...
var author$project$Bench$reverse = function _reverse (list) {
    return _foldl(_List_Cons, _List_Nil, list);
};
var elm$core$Basics$add = _Basics_add;
var elm$core$Basics$gt = _Utils_gt;
	function _foldrHelper (fn, acc, ctr, ls) {
		if (!ls.b) {
			return acc;
		} else {
			var a = ls.a;
			...
			if (!r1.b) {
				...
				if (!r2.b) {
					...
					if (!r3.b) {
						...
						var d = r3.a;
						var r4 = r3.b;
					    var res = (ctr > 500) ? _foldl(fn, acc, author$project$Bench$reverse(r4)) : _foldrHelper(fn, acc, ctr + 1, r4);
    ...
    var author$project$Bench$foldrHelper = F4(_foldrHelper);
	function _foldr (fn, acc, ls) {
	    return _foldrHelper(fn, acc, 0, ls);
	}
    var author$project$Bench$foldr = F3(_foldr);
	function _filter (isGood, list) {
	    return _foldr(
				function (x, xs) {
    ...
    var author$project$Bench$filter = F2(_filter);
	function _map (f, xs) {
	    return _foldr(
				function (x, acc) {
    ...
    var author$project$Bench$map = F2(_map);
			...
			[
				...
				_List_fromArray(
					[
						...
						'int +',
						function (_n2) {
						    return _foldl(elm$core$Basics$add.f, 0, intList);
						}),
						A2(
						elm_explorations$benchmark$Benchmark$benchmark,
						'string ++',
						function (_n3) {
						    return _foldl(elm$core$Basics$append.f, '', stringList);
						})
					])),
				A2(
				elm_explorations$benchmark$Benchmark$describe,
				...
				_List_fromArray(
					[
						...
						'String reverse',
						function (_n5) {
						    return _map(elm$core$String$reverse, stringList);
						})
					])),
				A2(
				elm_explorations$benchmark$Benchmark$describe,
